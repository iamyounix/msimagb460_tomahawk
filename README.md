...
- [Propertree](https://github.com/corpnewt/ProperTree) is a cross platform application to modify `.plist` file format. Requirement needed:
  ...
  - Linux, none. Most linux distro is built in with it. Use portable propertree instead. Arch users, just `git clone https://aur.archlinux.org/propertree-git.git`, `cd propertree-git` and `makepkg -si`.
