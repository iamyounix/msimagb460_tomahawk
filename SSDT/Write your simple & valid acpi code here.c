...
DefinitionBlock ("", "SSDT", 2, "MSI", "B460", 0x42343630) {
	...
	Scope (\_SB) {
		...
		Scope (PCI0) {
			 ...
			 * if the target operating system is Darwin, which is the core of the open-source
			 ...
			 * is Darwin (macOS), the code within the if statement will be executed, otherwise
			 * it will be ignored. This is simple workaround to fix directive device check after
			 * PCI0.
			 */
			...
			If (_OSI ("Darwin")) {
				...
				*/
				...
				Scope (RP03) {
					...
					Device (LAN1) {
						Name (_ADR, Zero) Method (_STA, 0, NotSerialized) {
							Return (0x0F)
						}
					}
				}
